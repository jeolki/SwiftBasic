...
/*
  1. 고차 함수
  - 고차 함수(Higher-order)는 '다른 함수를  전달인자로 받거나 함수실행의 결과를 함수로 반환하는 함수'를 뜻합니다.
  - 스위프트의 함수(클로저)는 일급시민(일급객체)이기 때문에 함수의 전달인자로 전달할 수 있으며, 함수의 결과값으로 반활할 수 있습니다.
...
// 변형하고자 하는 number와 변형 결과를 받을 doubledNumbers, strings
...
// number의 각 요소를 문자열로 변환하여 새로운 배열 반환
